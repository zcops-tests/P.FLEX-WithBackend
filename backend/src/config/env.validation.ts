...
export function validateEnv(config: Record<string, unknown>): AppConfig {
  return {
    ...
    REQUEST_BODY_LIMIT_MB: requireNumber(config.REQUEST_BODY_LIMIT_MB, 'REQUEST_BODY_LIMIT_MB', 25),
    DATABASE_URL: requireString(config.DATABASE_URL, 'DATABASE_URL'),
    REDIS_HOST: typeof config.REDIS_HOST === 'string' && config.REDIS_HOST.trim() !== '' ? config.REDIS_HOST : 'localhost',
    ...
    JWT_ACCESS_EXPIRATION:
      typeof config.JWT_ACCESS_EXPIRATION === 'string' && config.JWT_ACCESS_EXPIRATION.trim() !== ''
        ? config.JWT_ACCESS_EXPIRATION
        : '15m',
    JWT_REFRESH_EXPIRATION:
      typeof config.JWT_REFRESH_EXPIRATION === 'string' && config.JWT_REFRESH_EXPIRATION.trim() !== ''
